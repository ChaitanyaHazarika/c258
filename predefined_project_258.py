...
leftFrame  = Frame(root, bd  = 5, bg = "white", padx = 10, pady= 10)
Label(leftFrame , text = "Enter User Name").grid(row = 0, column= 0 , sticky=W , pady= 15)
Label(leftFrame , text = "Enter Password").grid(row = 1, column= 0 , sticky=W , pady= 15)
userID = Entry(leftFrame)
password = Entry(leftFrame, show="/")
userID.grid(row=0 , column=1, padx= 20, pady= 10)
...
def openNewWindow():
    ...
    if userName== "My account" and userPass == '123456':
...
loginButton = Button(leftFrame, text="Login", cursor="hand2", command=openNewWindow)
loginButton.grid(row = 2, column=1, padx=20, pady=20 )
